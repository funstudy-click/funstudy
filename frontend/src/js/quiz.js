    ...
    switch(e.key) {
        ...
        case 'ArrowDown':
            ...
            if (!document.getElementById('next-btn').disabled) {
                nextQuestion();
            } else if (document.getElementById('submit-btn').style.display !== 'none') {
                submitQuiz();
            }
            break;
        case '1':
...
// Auto-save progress to localStorage (optional feature)
...
function loadProgress() {
    try {
        ...
        if (savedProgress) {
            ...
            if (progressData.quizData && progressData.quizData.length > 0) {
                return progressData;
            }
        }
    } catch (error) {
...
// Auto-save progress when user answers questions
function autoSaveProgress() {
    // Save progress every time user answers a question
    const originalSelectOption = selectOption;
    selectOption = function(index, selectedAnswer) {
        originalSelectOption(index, selectedAnswer);
        saveProgress();
    };
}

// Initialize auto-save when quiz starts
document.addEventListener('DOMContentLoaded', function() {
    // Check for saved progress on page load
    const savedProgress = loadProgress();
    if (savedProgress) {
        // Ask user if they want to continue previous quiz
        if (confirm('You have a saved quiz in progress. Would you like to continue?')) {
            // Restore progress
            quizData = savedProgress.quizData;
            currentQuestionIndex = savedProgress.currentQuestionIndex;
            userAnswers = savedProgress.userAnswers;
            quizStartTime = new Date(savedProgress.quizStartTime);
            currentGrade = savedProgress.currentGrade;
            currentSubject = savedProgress.currentSubject;
            currentDifficulty = savedProgress.currentDifficulty;
            
            // Update UI
            document.getElementById('quiz-title').textContent = 
                `${currentGrade} ${currentSubject} (${currentDifficulty})`;
...
displayResults = function(results) {
    ...
    clearProgress(); // Clear saved progress when quiz is completed
};